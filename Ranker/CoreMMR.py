    ...
    def getRankedList(self, question):
        ...
        for i in range(self.numSelectedSentences):
            ...
            for sentence in sentences:
                ...
                ###print (question.body)
                ques_sim = similarity.calculateSimilarity_token_input(sentence.tokens, question.tokens)
                max_sent_sim = -99999999
                for other in best:
                    # similarityInstance = SimilarityJaccard(sentence, other)
                    sim = similarity.calculateSimilarity_token_input(sentence.tokens, other.tokens)
                    if self.beta != 0:
                        try:
